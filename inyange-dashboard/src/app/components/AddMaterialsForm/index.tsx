...
const AddMaterialModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const onSubmit: SubmitHandler<MaterialData> = async (data:any) => {
    setLoading(true);
    ...
    try {
      ...
      setFeedbackMessage(`Failed to add material: ${error.message || 'Unknown error'}`);
      setFeedbackType('error');
    } finally {
      setLoading(false);
    }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              />
              {errors.material_name && <p className="text-red-500 text-sm mt-1">{errors.material_name.message}</p>}
            </div>
