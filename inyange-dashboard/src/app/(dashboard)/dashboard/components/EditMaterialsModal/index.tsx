...
const EditMaterialModal = ({
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setImagePreview(
        materialData?.image && typeof materialData.image === "string"
          ? materialData.image
          : null
      );
    }
  ...
  const onSubmit: SubmitHandler<MaterialData> = async (data) => {
    ...
    try {
      const response = await editMaterial(
        materialData.material_id.toString(),
        formData
      );
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-5xl">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-[20px] font-bold text-[#263C5A]">Edit Material</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          ...
          <div className="flex justify-end mt-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`px-4 py-2 bg-[#F8B612] text-[#263C5A] font-bold rounded-lg hover:bg-[#E6A300] transition-colors duration-300 text-[15px] ${
