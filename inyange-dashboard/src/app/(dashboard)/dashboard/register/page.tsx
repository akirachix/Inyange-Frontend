    ...
    try {
      ...
      if (response && response.error) {
        ...
        setTimeout(() => {
          ...
          if (data.user_role === "homeowner") {
            router.push("/pwa/components/Homepage"); // Redirect to PWA for homeowners
          } else if (data.user_role === "supplier") {
            router.push("/dashboard"); // Redirect to dashboard for suppliers
          }
        }, 2000);
      }
    } catch (error) {
...
//           router.push("/dashboard/login"); // Change this to your login page route
//         }, 2000);
//       }
//     } catch (error) {
//       console.error("Registration error:", error);
...
//             <button
//               type="submit"
//               className="w-full py-3 lg:py-4 px-4 bg-[#263C5A] lg:px-6 text-base lg:text-lg font-semibold border border-transparent rounded-lg shadow-md text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-300"
//             >
//               Sign Up
//             </button>
//             {successMessage && (
